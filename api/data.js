...
export default async function handler(req, res) {
  ...
  // Init tables + add new columns if missing
  try {
  ...
  try {
    ...
    if (req.method === 'POST') {
      const b = req.body;
      let rows;
      if (table === 'members') rows = await sql`INSERT INTO members (first,last,type,title,series,email,phone,initials,card_bg,card_txt,card_gradient) VALUES (${b.first},${b.last},${b.type},${b.title},${b.series},${b.email},${b.phone},${b.initials||null},${b.card_bg||null},${b.card_txt||null},${b.card_gradient||null}) RETURNING *`;
      ...
      else if (table === 'bids') rows = await sql`INSERT INTO bids (num,member_id,gc_id,gc_name,contact_id,contact_name,job,value,date,follow_days,decision,status,notes,pdfs,company) VALUES (${b.num},${b.member_id},${b.gc_id},${b.gc_name},${b.contact_id},${b.contact_name},${b.job},${b.value},${b.date},${b.follow_days},${b.decision||null},${b.status},${b.notes},${JSON.stringify(b.pdfs||[])},${b.company||'ACI'}) RETURNING *`;
    ...
    if (req.method === 'PATCH') {
      ...
      let rows;
      if (table === 'members') rows = await sql`UPDATE members SET first=${b.first},last=${b.last},type=${b.type},title=${b.title},series=${b.series},email=${b.email},phone=${b.phone},initials=${b.initials||null},card_bg=${b.card_bg||null},card_txt=${b.card_txt||null},card_gradient=${b.card_gradient||null} WHERE id=${rid} RETURNING *`;
